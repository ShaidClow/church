...
$(document).ready(function() {
    ...
    $(this).css("box-shadow" , "0px 0px 0px rgba(50, 50, 50, 0.75)");
    removeOk();
}).mouseleave(function() { 
    $(this).css("-webkit-box-shadow"," 4px 4px 4px rgba(50, 50, 50, 0.5)"); 
    $(this).css("-moz-box-shadow", "4px 4px 4px rgba(50, 50, 50, 0.5)");
    $(this).css("box-shadow" , "4px 4px 4px rgba(50,50,50,0.5)"); 
    paintOk();
}).mousedown(function(){if (typeGame!=99){isCorrect();}})
initAct();})

function paintOk(){ 
    var canvas = document.getElementById("buttonOkCanvas"); 
    var contexto = canvas.getContext("2d");
        contexto.fillStyle =colorButton;
        contexto.fillRect(1,1,canvas.width-2,canvas.height-2);
    var x=canvas.width-2; 
    var grd = contexto.createRadialGradient(x/2, x/2,1, x/2, x/2, 30);
        grd.addColorStop(0,colorBack);
        grd.addColorStop(1,colorSele);
        contexto.fillStyle = grd; 
        contexto.fillRect(10, 10,canvas.width-20,canvas.height-20);
        contexto.lineWidth = 1; contexto.strokeStyle =colorText;
        contexto.fillStyle =colorText;contexto.beginPath();
        contexto.strokeRect(10, 10,canvas.width-20,canvas.height-20);
        contexto.font="bold 22px Verdana"; 
        contexto.textAlign = "left"; 
        contexto.fillText("Contestar",25,48); 
        contexto.stroke();
    }
...
function paintTab(){
    if (document.getElementById("ardoraTabCanvas")!=null){
        ...
        var endAngle = 1.5*Math.PI;
            ...
            contexto.fill();;
            contexto.lineWidth = 5;
             if (timeAct>timeIni/4){c
                ontexto.strokeStyle =colorSele;
            }else{
        ...
        if (tiSuccesses){
            ...
            contexto.font="8px Verdana";
            contexto.textAlign = "left";
            contexto.fillText("ACIERTOS:",12,profundidade+14);
            contexto.lineWidth = 5;
            contexto.stroke();
            contexto.beginPath();
            contexto.fillStyle =colorBack;
...
function printJustify(context, text, x, y, lineHeight, fitWidth) { 
    ...
    while (words.length > 0 && idx <= words.length) { 
        ...
        if (w > fitWidth) { 
            ...
            outLines.push(words.slice(0, idx - 1).join(" "));
            currentLine++;
    ...
    for (i=0; i<outLines.length; i++){ 
        if (context.measureText(outLines[i]).width>maxW){
            maxW=context.measureText(outLines[i]).width}
        }
    for (i=0; i<outLines.length-1; i++){ 
        var wordsLine = outLines[i].split(" "); 
        var indexB=1; 
        var space="  "; 
        while (context.measureText(outLines[i]).width<maxW && indexB>0){ 
            var newLine=""; 
            for (j=0; j<wordsLine.length; j++){ 
                if (j<indexB){
                newLine=newLine+wordsLine[j]+space;
            } else{ 
                newLine=newLine+wordsLine[j]+" ";
            }
        } 
        outLines[i]=$.trim(newLine);
        indexB+=1;
        if (indexB>wordsLine.length){
            indexB=1; 
            space=space+" "
        } if (i==outLines.length-2){
            indexB=0;
        }
    }
} return outLines;}
...
function roundedRect(ctx,x,y,width,height,radius,color){ 
    ...
    if (color!=""){
        if (color.substr(0,9)=="gradient_"){ 
            var my_gradient=ctx.createLinearGradient(x+width/2,y,x+width/2,y+height); 
                my_gradient.addColorStop(0,color.substr(9,20)); 
...
while (!((dx < 0 ? x <= toX : x >= toX) && (dy < 0 ? y <= toY : y >= toY))) {var dashLength = dashPattern[idx++ % dashPattern.length];
...
function paintBubble(ctx, x, y, w, h, xPointer, radius) {
    if (xPointer == 0) {
        ...
        ctx.fillStyle = colorSele;
        ctx.globalAlpha = 0.7;
        ctx.beginPath();
        ctx.strokeStyle = colorSele;     
        ctx.lineWidth = "2";
        roundedRect(ctx, x, y, w, h - 10, 10, colorSele);
        ctx.moveTo(xP, y + h - 10);
        ctx.lineTo(xP + 10, y + h - 10);
        ctx.lineTo(xP + 5, y + h);
        ctx.lineTo(xP, y + h - 10);
        ctx.shadowColor = "#999";
        ctx.shadowBlur = 5;
        ctx.shadowOffsetX = 3;
        ctx.shadowOffsetY = 3;
...
function showMessage(typeMessage){ 
    var oldTypeGame=typeGame;
    typeGame=99;
    isShowMessage=true;
    
    if (typeMessage=="Ok"){ 
        var textMessage=messageOk;
        var urlMessage=urlOk;
        var goURLTarget= goURLOk;
        var borderColor=borderOk;
    }
    if (typeMessage == "scoMessage") {
        var textMessage = scoMessage;
        var urlMessage = "";
        var goURLTarget = "";
        var borderColor = borderAttempts;
    }
    if (typeMessage=="Time"){
        var textMessage=messageTime;
        var urlMessage=urlTime; 
        var goURLTarget= goURLTime; 
        var borderColor=borderTime;
    }
...
contexto.fillRect(0,0,canvas.width,canvas.height); contexto.font="26px " + fMenssage; var metricsW = contexto.measureText(textMessage).width;var x=(canvas.width / 2)-(metricsW / 2);var y=(canvas.height / 2);
...
contexto.font="26px " + fMenssage ;var metricsW = contexto.measureText(textButtonTime).width;var x=(canvas.width / 2)-(metricsW / 2);var y=(canvas.height / 2);
